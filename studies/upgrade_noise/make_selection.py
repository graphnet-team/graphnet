...
def parallel_this(settings):
    ...
    for index in tqdm.tqdm(events):
        features = con.execute("SELECT event_no FROM SplitInIcePulses_GraphSage_Pulses WHERE event_no = {}".format(index))
...
def make_even_track_cascade(events, db):
    with sqlite3.connect(db) as con:
        query = 'select event_no from truth where abs(pid) = 14 and interaction_type = 1 and event_no in %s'%str(tuple(events['event_no']))
        tracks = pd.read_sql(query,con)
        query = 'select event_no from truth where event_no not in %s and event_no in %s'%(str(tuple(tracks['event_no'])), str(tuple(events['event_no'])))
    ...
    if len(tracks) > len(cascades):
        return pd.concat([tracks.sample(len(cascades)), cascades], ignore_index = True).sample(frac = 1).reset_index(drop = True)
    else:
        return pd.concat([tracks, cascades.sample(len(tracks))], ignore_index = True).sample(frac = 1).reset_index(drop = True)
...
if __name__ == '__main__':
    ...
    events = pd.read_csv(path + '/over10pulses.csv',)
