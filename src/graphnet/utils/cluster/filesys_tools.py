...
def remove_files(file_list, prompt=True, remove_dirs=False) :
    ...
    print("%i files%s to remove :" % (len(file_list), "/directories" if remove_dirs else "") )
    ...
    if prompt :
        delete_files = False
        while True :
            response = input( "Proceed with deleting these %i files%s? [y/n]\n" % (len(file_list), "/directories" if remove_dirs else "") )
            if response.lower() == "y" :
                delete_files = True
                ...
                print("Unrecognised response '%s', choose from [y/n]" % response)
    else :
        delete_files = True
    ...
    if delete_files :
        for f in file_list :
            if os.path.isfile(f) :
                ...
                assert remove_dirs == True, "Found a directory to delete, but `remove_dirs` is False"
                shutil.rmtree(f)
