...
class default_loss_calc(Model):
    """A module that applies the default loss logic that matches the default augment."""
    ...
    def forward(self, 
                ...
                data: Data, 
                aux: Tuple[Tensor, Tensor, Tensor]) -> Tensor:
        """Forward pass."""
        lat, cls_tensor = pred
...
default_task = UnsupervisedTask(default_mask_augment(),
                                default_loss_calc())
...
class mask_pred_frame_new(EasySyntax):
    """The BERT-Style mask prediction module.
    ...
    def __init__(
        self,
        encoder: Model,
        bert_task: UnsupervisedTask = default_task,
        encoder_out_dim: Optional[int] = None,
        need_charge_rep: bool = False,
        optimizer_class: Type[torch.optim.Optimizer] = Adam,
        optimizer_kwargs: Optional[Dict] = None,
