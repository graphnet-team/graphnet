...
def convert_data(data: Data) -> tuple:
    ...
    Returns:
        x: Reshaped input tensor [B x L x D].
        mask: The mask tensor.
        seq_length: The actual number of pulses tensor
        (after sub-sampling).
    """
    _, seq_length = torch.unique(data.batch, return_counts=True)
    x_list = torch.split(data.x, seq_length.tolist())
    x = torch.nn.utils.rnn.pad_sequence(
        x_list, batch_first=True, padding_value=torch.inf
    )
...
class DeepIceWithDynEdge(GNN):
    ...
    def __init__(
        ...
        Args:
            ...
            depth: The depth of the transformer.
            head_size: The size of the attention heads.
            nb_neighbours: Number of neighbours to used in the knn graph.
        ...
        self.dyn_edge = DynEdge(
            nb_inputs=9,  # 9 features, TBD by user
