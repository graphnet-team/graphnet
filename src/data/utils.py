...
def extract_features(frame, pulsemap, gcd_dict,calibration):
...
def if_available(expression,frame, padding_value = -1):
    try:
        eval(expression)
        out = expression
    except:
...
def oscnext_extractor(frame, pulsemap, gcd_dict, calibration, input_file, custom_truth = None):
