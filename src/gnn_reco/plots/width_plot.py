    ...
    if track_cascade == False:
        for key in biases['dynedge'].keys():
            ...
            if len(plot_data['mean']) != 0:
                ...
                if include_retro:
                    ax2.errorbar(plot_data['mean'],1 - np.array(plot_data['width'])/np.array(plot_data_retro['width']),calculate_relative_improvement_error(1 - np.array(plot_data['width'])/np.array(plot_data_retro['width']), plot_data['width'], plot_data['width_error'], plot_data_retro['width'], plot_data_retro['width_error']),marker='o', capsize = 10,markeredgecolor='black')
        ...
        for key in biases['dynedge'].keys():
            ...
            if len(plot_data_track['mean']) != 0:
                ...
                if include_retro:
                    ax2.errorbar(plot_data_track['mean'],1 - np.array(plot_data_track['width'])/np.array(plot_data_retro_track['width']),calculate_relative_improvement_error(1 - np.array(plot_data_track['width'])/np.array(plot_data_retro_track['width']), plot_data_track['width'], plot_data_track['width_error'], plot_data_retro_track['width'], plot_data_retro_track['width_error']),marker='o', capsize = 10,markeredgecolor='black', color = 'limegreen', label = 'track',linestyle='dotted')
                    ax2.errorbar(plot_data_cascade['mean'],1 - np.array(plot_data_cascade['width'])/np.array(plot_data_retro_cascade['width']),calculate_relative_improvement_error(1 - np.array(plot_data_cascade['width'])/np.array(plot_data_retro_cascade['width']), plot_data_cascade['width'], plot_data_cascade['width_error'], plot_data_retro_cascade['width'], plot_data_retro_cascade['width_error']),marker='o', capsize = 10,markeredgecolor='black', color = 'springgreen', label = 'cascade',linestyle='solid')
                    ax2.legend()
                #plt.title('$\\nu_{v,u,e}$', size = 20)
                ax1.tick_params(axis='x', labelsize=10)
                ax1.tick_params(axis='y', labelsize=10)
