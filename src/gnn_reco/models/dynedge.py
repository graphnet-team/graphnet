...
class dynedge_energy_xfeats(torch.nn.Module):                                                     
    def __init__(self,k, device, n_outputs):                                                                                   
        ...
        self.nn3 =  torch.nn.Linear(4*l5 + 8,l6)
    ...
    def forward(self, data):
        ...
        x2 = torch.cat((a,b,c,d, data.h_t.reshape(-1,1), data.h_x.reshape(-1,1), data.h_y.reshape(-1,1), data.h_z.reshape(-1,1), data.h_c.reshape(-1,1), data.h_rde.reshape(-1,1), data.h_pmt.reshape(-1,1), data.n_pulses.reshape(-1,1)),dim = 1)#, h_t, h_x, h_y, h_z), dim = 1)
        x2 = self.relu(x2)
        x2 = self.nn3(x2)
