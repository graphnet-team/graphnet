...
def build_module_dictionary(targets):
    ...
    for target in targets:
        module_dict[target] = {}
        module_dict[target]['path'] = '/home/iwsatlas1/oersoe/phd/oscillations/models/final/dynedge_oscillation_final_%s.pth'%(target) #dev_lvl7_robustness_muon_neutrino_0000/dynedge_oscillation_%s/dynedge_oscillation_%s.pth'%(target,target)
...
try:
    ...
    pass
# Build Pipeline
pipeline = InSQLitePipeline(module_dict = build_module_dictionary(targets), 
                            features = features, 
                            truth = truth, 
                            device = device,
