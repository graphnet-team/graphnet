...
with ClusterSubmitter(
        ...
        num_cpus=2,
        num_gpus=1,
        memory=num_cpus*4096, # most clusters have 4gb memory allocated per cpu
        disk_space=1000,
        submit_dir="./tmp",
        output_dir=os.path.join(os.path.expanduser('~'),'graphnet','results'),
        run_locally=False,
        ...
        submitter.add(command="sh /groups/icecube/qgf305/graphnet/examples/train_model_cluster.sh",description="A description",allowed_return_status=[5,0]) # Define acceptable return status values